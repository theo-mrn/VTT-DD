// Crée backend/identity/.env depuis .env.example, avec une clé de signature JWT
...
if (existsSync(cible)) {
  console.log('backend/identity/.env existe déjà, conservé');
  process.exit(0);
}
...
console.log('backend/identity/.env créé avec une clé JWT de dev');
