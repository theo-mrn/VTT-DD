...
function useCanvasLock(wants: boolean) {
  ...
  useEffect(() => {
    ...
    return () => {
      canvasListeners.delete(rerender);
    };
  }, []);
  useEffect(() => {
    ...
    return () => {
      if (activeCanvasId === idRef.current) {
        activeCanvasId = null;
        notifyCanvas();
      }
    };
 ...
 */
export function DicePreviewCard({
  skinId,
  type = 'd20',
  active,
}: {
  ...
  const handleCapture = (dataUrl: string) => {
    if (dataUrl) {
      snapshotCache.set(skinId, dataUrl);
      setSnapshot(dataUrl);
    }
  };

  const onImgError = () => {
    prebakedMissing.add(skinId);
    setPrebakedFailed(true);
  };
...
export function DiceGridModal({
  ...
  return (
    ...
    >
      <div
        className="bg-gray-900 rounded-xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden mx-4"
