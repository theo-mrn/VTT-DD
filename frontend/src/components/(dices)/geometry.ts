...
export const getDieValue = (type: string, index: number) => {
  ...
  if (type === 'd20') {
    ...
    const verifiedD20Map = [2, 20, 14, 6, 12, 8, 18, 17, 15, 10, 5, 19, 1, 7, 9, 16, 11, 13, 3, 4];
...
export const createBeveledGeometry = (type: string) => {
  ...
  switch (type) {
    case 'd4':
      geo = new THREE.OctahedronGeometry(1.8, detail);
      break;
    case 'd6':
      geo = new THREE.BoxGeometry(2.2, 2.2, 2.2, 2, 2, 2);
      break;
    case 'd8':
      geo = new THREE.OctahedronGeometry(1.8, detail);
      break;
    case 'd10':
      geo = new THREE.IcosahedronGeometry(1.8, detail);
      break;
    case 'd12':
      geo = new THREE.DodecahedronGeometry(1.8, detail);
      break;
    case 'd20':
      geo = new THREE.IcosahedronGeometry(1.8, detail);
      break;
  ...
  for (let i = 0; i < position.count; i += 3) {
    ...
    let found = groupedFaces.find((g) => g.normal.dot(normal) > 0.9);
    if (found) {
