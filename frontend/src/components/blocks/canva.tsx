...
export function CanvaSection({ onStart, isUserLoggedIn = null }: CanvaSectionProps) {
  ...
  const [coverRect, setCoverRect] = React.useState<{
    renderWidth: number;
    renderHeight: number;
    offsetX: number;
    offsetY: number;
  ...
  const toScreenPixels = React.useCallback(
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col items-start justify-center px-6 lg:px-16 text-left pointer-events-none z-10">
          ...
          >
            Créez, jouez et vivez vos aventures sur une table virtuelle complète, directement dans
            votre navigateur.
          </motion.p>
          <motion.div
