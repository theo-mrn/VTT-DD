...
- **Paquet `packages/rules`** : le moteur de règles et les systèmes de jeu (aujourd'hui `apps/web/src/lib/rules-engine` et `src/modules`, testés) deviennent un paquet partagé par le front et le service character, qui recalcule les stats dérivées côté serveur.
- **Nouveau front** : une nouvelle app Next dans le monorepo, construite par tranche ; `apps/web` reste lançable comme référence jusqu'à la parité, puis est supprimé.
...
Valkey et NATS ne sont pas sauvegardés : le cache se reconstruit, les événements sont déjà dans `history.events`. Le runbook est dans `deploy/platform/backup/RESTORE.md`. La clé de chiffrement des dumps doit aussi être rangée hors du cluster.
...
| 2. packages/rules           | Moteur de règles et systèmes de jeu extraits d'`apps/web` avec leurs tests                                                                                                                                                                                                                                                   | Tests verts dans le paquet, `apps/web` l'importe                                 |
...
| 8. Bascule                  | Import final des données, DNS vers le nouveau front, suppression d'`apps/web`, de Firebase et de ses variables                                                                                                                                                                                                               | `firebase` absent du lockfile, projet Firebase supprimé                          |
...
| Front               | **Décidé** : nouvelle app à côté d'`apps/web`, supprimé à la parité                                                                           | Réécriture dans `apps/web`                               |
