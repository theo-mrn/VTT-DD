...
const RARITY_STYLE: Record<string, { label: string; className: string }> = {
    legendary: { label: 'Légendaire', className: 'text-[var(--accent-brown)] border-[color-mix(in_srgb,var(--accent-brown)_40%,transparent)] bg-[color-mix(in_srgb,var(--accent-brown)_10%,transparent)]' },
    epic: { label: 'Épique', className: 'text-[var(--accent-blue)] border-[color-mix(in_srgb,var(--accent-blue)_40%,transparent)] bg-[color-mix(in_srgb,var(--accent-blue)_10%,transparent)]' },
...
export function DiceDetail({ skin, isOwned, isEquipped, canAfford, onBack, onBuy, onEquip, onTry }: DiceDetailProps) {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <button
                onClick={onBack}
                className="mb-5 flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[color-mix(in_srgb,var(--text-primary)_5%,transparent)] transition-colors"
            >
            ...
            <div className="grid md:grid-cols-2 gap-6 items-start">
                ...
                <div className="relative aspect-square rounded-2xl overflow-hidden border border-[var(--border-color)] bg-[var(--bg-darker)]">
                    ...
                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[9px] font-bold uppercase tracking-widest text-[var(--text-secondary)] pointer-events-none" style={{ background: 'color-mix(in srgb, var(--bg-dark) 70%, transparent)', borderColor: 'color-mix(in srgb, var(--border-color) 50%, transparent)' }}>
                ...
                <div className="flex flex-col gap-4 md:pt-2">
                    ...
                    <div className="mt-2 pt-4 border-t border-[var(--border-color)] flex flex-col gap-3">
                        {isOwned ? (
                            <button
                                ...
                                className={cn(
                                    ...
                                    isEquipped
                                        ? 'bg-[color-mix(in_srgb,var(--text-primary)_5%,transparent)] text-[var(--text-secondary)] cursor-not-allowed'
                                        : 'bg-[var(--accent-brown)] text-[var(--bg-dark)] hover:bg-[var(--accent-brown-hover)] active:scale-95 shadow-lg shadow-[color-mix(in_srgb,var(--accent-brown)_10%,transparent)]'
                                )}
                            >
                        ...
                        <button
                            onClick={onTry}
                            className="w-full py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-colors flex items-center justify-center gap-2 border border-[var(--border-color)] text-[var(--text-secondary)] hover:text-[var(--accent-brown)] hover:border-[color-mix(in_srgb,var(--accent-brown)_40%,transparent)] active:scale-95"
                        >
